...
def _handle_capture(body: dict[str, Any], rid: str, agent: str) -> dict[str, Any]:
    ...
    capture_size: list[int] = body.get("capture_size", [640, 640])
    cmd: list[str] = [
        sys.executable, str(WIN32_PATH), "capture",
        "--region", region,
    ...
    def do_POST(self) -> None:
        ...
        if path == "/route":
            ...
            with _pending_lock:
                ...
                stale: list[str] = [k for k, v in _pending.items() if time.time() - v["ts"] > 30.0]
                for k in stale:
                    _pending.pop(k)
            if slot:
